#!/usr/bin/env python3
"""A projection block is another type of
building block used in ResNet architectures,
specifically when the dimensions of the input
and output features differ. It's designed to
handle situations where the number of
channels or spatial dimensions needs to
be changed."""
...
def projection_block(A_prev, filters, s=2):
    """Function that builds a projection bock
        A_prev is the output de the previous
        layer
        ...
        F11, F3, F12
            ...
            the second 1x1 convolution as well
            as the 1x1 convolution in the
            shortcut connection
        s is the stride of the first
        convolution in both the main path
        and the shortcut connection
        All convolutions inside the block
        should be followed by batch
        normalization along the channels
        axis and a ReLU activation
        All weights should use he normal
        initialization
        The seed for the he_normal
    ...
    shortcut_conv = K.layers.Conv2D(filters=F12,
                                    kernel_size=(1, 1),
                                    strides=(s, s),
                                    padding='valid',
                                    kernel_initializer=init)(A_prev)
