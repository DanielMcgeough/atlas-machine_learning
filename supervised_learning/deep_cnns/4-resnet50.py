#!/usr/bin/env python3
"""This architecture follows rules
the only issues it that the rules are constantly
chaning up on me."""
...
def resnet50():
    """Function that builds the ResNet-5
    architecture
        Assume the input data will have
        shape (224, 224, 3)
        All convolutions inside and outside
        the blocks should be followed by
        batch normalization along the
        channels axis and a ReLU activation
    ...
    avg_pool = K.layers.AveragePooling2D(pool_size=(7, 7))(id_block12)
