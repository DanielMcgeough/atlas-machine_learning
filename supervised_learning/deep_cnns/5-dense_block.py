#!/usr/bin/env python3
"""A dense block is a fundamental building
block used in DenseNet architectures.
It's designed to connect each layer to
every other layer in a feed-forward fashion,
promoting feature reuse and efficient
information flow. """
...
def dense_block(X, nb_filters, growth_rate, layers):
    """Function that builds a dense block
        X is the output from the previous
        layer
        nb_filters is an i nteger representing
        the number of filters in X
        grownth_rate is the growth rate for
        the dense bloack
        layers is the number of layers in
        the dense block
        You should use the bottleneck layers
        used for DenseNet-B
        All weights should use he_normal
        initialization
        The seed for the he_normal initializer
        should be set to zero
        All convolutions should be preceded
        by Batch Normalization and a
        ReLU activation

        Returns: The concatenated output of
        each layer within the Dense Block
        and the number of filters within
